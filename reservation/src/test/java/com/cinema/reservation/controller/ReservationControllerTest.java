...
class ReservationControllerTest {
    ...
    private RoomResponse createRoomWithThreeSeats(int roomNumber) {
        return RoomResponse.builder().roomNumber(roomNumber).seatsNumber(3).seats(Arrays.asList(
    ...
    void shouldCreateReservation() throws Exception {
        Mockito.when(roomsFeignClient.getRoomByRoomNumber(1)).thenReturn(new ResponseEntity<>(createRoomWithThreeSeats(1), HttpStatus.OK));
        ...
        Mockito.when(moviesFeignClient.getByTitle(MOVIE_TITLE)).thenReturn(new ResponseEntity<>(HttpStatus.OK));
    ...
    void shouldCancelReservation() throws Exception {
        ...
        Mockito.when(roomsFeignClient.changeSeatReservation(any())).thenReturn(null);
        Mockito.when(reservationCodeGenerator.generateCode()).thenReturn(RESERVATION_CODE_A).thenReturn(RESERVATION_CODE_B);
        Mockito.when(moviesFeignClient.getByTitle(MOVIE_TITLE))
                .thenReturn(new ResponseEntity<>(new MovieResponse(), HttpStatus.OK))
                .thenReturn(new ResponseEntity<>(new MovieResponse(), HttpStatus.OK));
        ...
        mockMvc.perform(delete("/reservation/cancelReservation/" + RESERVATION_CODE_A)
                        .contentType("application/json"))
                .andExpect(status().isOk());
    ...
    void shouldGetClients() throws Exception {
        ...
        Mockito.when(roomsFeignClient.changeSeatReservation(any())).thenReturn(null);
        Mockito.when(reservationCodeGenerator.generateCode()).thenReturn(RESERVATION_CODE_A).thenReturn(RESERVATION_CODE_B);
        Mockito.when(moviesFeignClient.getByTitle(MOVIE_TITLE)).thenReturn(new ResponseEntity<>(new MovieResponse(), HttpStatus.OK));
